...
DBNAME=${PGDATABASE:-gis_carto_v6}
STYLE_DIR=${STYLE_DIR:-/opt/openstreetmap-carto}
DATABASE_STATE_DIR=${DATABASE_STATE_DIR:-/data/database18-carto-v6-state}
OSM2PGSQL_OPTIONS="--append -O flex -S $STYLE_DIR/openstreetmap-carto-flex.lua -d $DBNAME --number-processes ${THREADS:-4} ${OSM2PGSQL_EXTRA_ARGS}"

# flat-nodes
if [ -f "$DATABASE_STATE_DIR/flat_nodes.bin" ]; then
    OSM2PGSQL_OPTIONS="${OSM2PGSQL_OPTIONS} --flat-nodes $DATABASE_STATE_DIR/flat_nodes.bin"
fi
...
#------------------------------------------------------------------------------
TRIM_POLY_FILE="$DATABASE_STATE_DIR/region.poly"
TRIM_OPTIONS="-d $DBNAME"
TRIM_REGION_OPTIONS="-p $TRIM_POLY_FILE"

BASE_DIR=$DATABASE_STATE_DIR
...
#------------------------------------------------------------------------------
if ! render_expired --map=standard --min-zoom=$EXPIRY_MINZOOM --touch-from=$EXPIRY_TOUCHFROM --delete-from=$EXPIRY_DELETEFROM --max-zoom=$EXPIRY_MAXZOOM -s /run/renderd/renderd.sock < "$EXPIRY_FILE.$$" 2>&1 | tail -8 >> "$EXPIRYLOG"; then
