...
grep -Fq 'openmaptiles-sources.sha256' "$BUILDER"
! grep -Fq -- ' --download ' "$BUILDER"
grep -Fq 'dev.maptiler.download/geodata/omt/lake_centerline.shp.zip' "$ROOT/scripts/fetch-openmaptiles-sources.sh"
grep -Fq 'osmdata.openstreetmap.de/download/water-polygons-split-3857.zip' "$ROOT/scripts/fetch-openmaptiles-sources.sh"
